...
class Main {
  ...
  async ticker_delete(userId, tickerId) {
    ...
    try {
      await this.pool.query('DELETE FROM user_tickers WHERE user_id = $1, ticker_id = $2', [userId, tickerId]);
      return { statusCode: 200 };
    } catch (error) {
      return { statusCode: 500, error: error };
    }
  ...
  async ticker_list(userId, start = 0, limit = 100) {
    if (typeof start !== 'number' || typeof limit !== 'number') {
      return { statusCode: 400 };
    }
    try {
      const tickerIds = (
        await this.pool.query('SELECT ticker_id FROM user_tickers WHERE user_id = $1 OFFSET $2 LIMIT $3', [userId, start, limit])
      ).rows.map((obj) => obj.ticker_id);
      const tickerIdsAsParams = tickerIds.map((_, i) => `$${i + 1}`).join(',');
      const resultone = (await this.pool.query(`SELECT * FROM tickers WHERE id IN (${tickerIdsAsParams})`, tickerIds)).rows;

      const resulttwo = await this.pool.query('SELECT * FROM ticker_data')
